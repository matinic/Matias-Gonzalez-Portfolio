...
export default function Email() {
  ...
  const verification: ()=>void = ()=>{
    ...
    if(!dataForm.current.user_name) errorMessage.current = {...errorMessage.current, user_name: msgText.user}
    if(!dataForm.current.from_email) errorMessage.current = {...errorMessage.current, from_email: msgText.email}
    if(!dataForm.current.message) errorMessage.current = {...errorMessage.current, message: msgText.message}
  }

  const validation = (e:React.FormEvent<HTMLInputElement | HTMLTextAreaElement>)=>{
