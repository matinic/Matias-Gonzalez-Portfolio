...
export default function Navbar() {
  const menuButtons: Array<Array<string>> = [
    ["Sobre Mi", "sobremi"],
    ["Conocimientos", "conocimientos"],
    ["Proyectos", "proyectos"],
    ["Contacto", "contacto"],
    ["Descargar CV", "descargarcv"],
  ];
  ...
  const navOptions = (event: React.MouseEvent<HTMLElement>) => {
    ...
    if (key) {
      ...
      const scroll = elementToScrollTo
        ? elementToScrollTo.offsetTop - 56
        : undefined;
      window.scroll({
        top: scroll,
        behavior: "smooth",
  ...
  return (
    <div className="z-50">
      ...
      />
      <label
        className="w-12 h-12 bg-menu-open fixed bg-[length:2.5rem_2.5rem] bg-no-repeat bg-center z-40  peer-checked:bg-menu-close sm:hidden"
        htmlFor="showmenu"
      ></label>
